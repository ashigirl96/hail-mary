...
use crate::application::use_cases::{remember_memory, recall_memory};
...
impl MemoryService {
    ...
    pub fn remember_batch(&mut self, memories: Vec<MemoryInput>) -> Result<Vec<crate::domain::entities::memory::Memory>, ApplicationError> {
        let requests: Vec<RememberRequest> = memories.into_iter().map(|m| RememberRequest {
    ...
    pub fn recall(&mut self, query: &str, limit: usize, type_filter: Option<String>, tag_filter: Vec<String>) -> Result<String, ApplicationError> {
...
impl MemoryMcpServer {
    ...
    pub async fn remember(
        ...
        let memories = service.remember_batch(params.0.memories)
            .map_err(|e| McpError::internal_error(format!("Failed to store memories: {}", e), None))?;
    ...
    pub async fn recall(
        ...
        ).map_err(|e| McpError::internal_error(format!("Failed to recall memories: {}", e), None))?;
...
mod tests {
    ...
    fn test_memory_service_recall_with_filters() {
        // Red: テスト先行 - フィルターでの検索
        let memories = vec![
            Memory::new("tech".to_string(), "Tech Memory".to_string(), "Content".to_string())
                .with_tags(vec!["important".to_string()])
                .with_confidence(Confidence::new(0.9).unwrap()),
            Memory::new("domain".to_string(), "Domain Memory".to_string(), "Content".to_string())
        ...
        let result = service.recall("memory", 10, Some("tech".to_string()), vec!["important".to_string()]);
        assert!(result.is_ok());
