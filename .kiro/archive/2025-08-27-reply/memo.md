...
`crates/<name>/lib.rs` に以下のコードを追加してほしい。
examplesにこのコードを呼ぶ出すサンプルを書いてほしい
...
async fn load_auth() -> Result<OAuthAuth> {
...
async fn refresh_token(auth: &mut OAuthAuth) -> Result<()> {
...
refresh_token(auth).await?;
    ...
    let access_token = auth.access.as_ref()
