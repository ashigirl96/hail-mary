...
**新しいslash command(`/spec:investigate` )を作成したいので、相談に乗ってください** 。
objective:
- `/spec:requirements` (; @.claude/commands/hm/requirements.md ) でPRDやbug reportの追加調査するため
- この後作る `/spec:design` (まだ作ってない) で設計をするための調査するため
...
- `/spec:requirements` のように、interactiveに調査を進める
...
- `/spec:investigate --topic` したら、
  - そのtopicに関する調査をする。
  - そこから、インタラクティブに会話をしながら、その `topic` が１つのsectionになるようにしたい
  - また `/spec:investigate --topic "YYY"` をしたら、以前の分客とは切り出して、新たなtopicとして、調査をして、それがまた１つのsectionとなるようにする
...
- `/spec:requirements` と `/spec:design` との関係性。可能な限り、 `/spec:requirements` と `/spec:design` はそれぞれ独立して、documentを作ることに集中して、 `/spec:investigate` がそれらの情報を補完するような役割をする？
  -  `/spec:investigate --requirements --topic` にしたら、<kiro_investigation_path> にも書き込むし、その後に、 <kiro_requirements_path> にも書き込むようにする?
  - `/spec:investigate --design --topic` にしたら、<kiro_investigation_path> にも書き込むし、その後に、 <kiro_design_path> にも書き込むようにする?
  - ↑はもっとgood ideaがあれば、提案がほしい
- `--topic` と書いてきたけど、調査してほしい内容を1行でまとめるのって難しいから、
  ```
  /spec:investigate --topic
...
- Task Toolの話もしたい
- `Example 3: Multi-Topic Session` がおかしい。`/spec:investigate --topic` で会話して、保存が終わった後に、また `/spec:investigate --topic` が実行されたら、「前のtopic？」と聞くんじゃなくて、新しいtopicを調査したい
...
- /spec:investigate しただけなのに、requirements.md とかdesign.mdを見に行ってた。 `--for` をつけてない場合は探さないでほしい（Will Notに書く内容？)
- 以下のように表示されているのに、
```
